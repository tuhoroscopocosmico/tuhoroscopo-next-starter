...
export async function GET() {
  ...
  const config = configRows.map((row) => ({
    ...
    editable: ["APP_DEBUG_MODE", "WHATSAPP_MODO", "THC_BACK_URL", "TTC_BACK_URL", "MODO_MANTENIMIENTO", "THC_PRECIO_SUSCRIPCION", "OPENAI_MODEL", "OPENAI_TEMPERATURE", "OPENAI_MAX_TOKENS"].includes(String(row.nombre).toUpperCase()),
  }));
